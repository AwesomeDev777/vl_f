...
const BackdropTransition = (props: any) => {
	return <Fade {...props} timeout={null} />
}
...
const Modal: BsPrefixRefForwardingComponent<'div', ModalProps> = React.forwardRef(
	(
		...
		const mergedRef = useMergedRefs(ref, setModalRef as React.Ref<unknown>)
		...
		const handleWindowResize = useEventCallback(() => {
			if (modal) {
				updateDialogStyle(modal.dialog as HTMLElement)
			}
		})
		...
		const handleStaticBackdropClick: React.MouseEventHandler<HTMLDivElement> = (e) => {
		...
		const renderDialog: React.ElementType = (dialogProps) => (
			<div
				role="dialog"
				{...dialogProps}
